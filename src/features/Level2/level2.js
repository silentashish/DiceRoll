...
const Level2 = props => {
  ...
  const [allOptions, setAllOptions] = useState([4, 3, 2]);
  ...
  const rollDice = () => {
    if (diceRollNumber < 20) {
      ...
      const sortedOptions = shuffle([answer, num1, num2]);
  ...
  const selectAnswer = selectedValue => {
    if (diceRollNumber < 20) {
      ...
      //Setting answers at random position
      const sortedOptions = shuffle([randomNumber, num1, num2]);
      setAllOptions(sortedOptions);
  ...
  function limitExceed() {
    if (score > 6) {
      Alert.alert(
        'Congratulations',
        'You have successfully passed level 2.',
        [
          {
            text: 'Cancel',
  ...
  return (
    <View style={{flex: 1, backgroundColor: 'white'}}>
      ...
      <Text style={styles.subHeading2}>LEVEL 2</Text>
